async function buscaTurma(turno, turma) {
  try {
    const res = await fetch(`https://devflow-1sem.up.railway.app/agenda`, {
      method: "POST",
      headers: {
    ...
    mensagemTexto.textContent =
      "Erro ao buscar dados do servidor. Tente novamente.";
    mensagemErro.classList.remove("hidden");
...
async function RenderTabela(dados) {
  ...
  const diasSemanaAula = ["Segunda", "Terça", "Quarta", "Quinta", "Sexta"];

  diasSemanaAula.forEach((dia) => {
    ...
    horariosUnicos.forEach((horario) => {
      const aula = dados.find(
        (item) =>
          item.diaSemana === dia && formatarHora(item.horario) === horario
      );
...
async function Filtro(curso) {
  try {
    const res = await fetch(`https://devflow-1sem.up.railway.app/busca-turma`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  Object.entries(dados).forEach(([turno, turmas]) => {
    ...
    btnTurno.addEventListener("click", () => {
      if (subSubMenu.classList.contains("active")) {
        ...
        document
          .querySelectorAll(".sub-sub-menu-curso.active")
